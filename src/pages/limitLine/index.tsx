...
const elementMap: Map<number, { rect: any, index: number }> = new Map()
...
const LimitLine = () => {
  ...
  const hideMoreElement = (line: number) => {
    ...
    setElementContainerHeight(oneLineHeight * line + 'px');
  };
   ...
   */
  const handleExpand = () => {
    console.log('点击展开按钮', elementMap.size > maxLine);
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      elementMap.set(rect.bottom, { rect, index: i }); // 记录每一行最后一个元素element，元素的下标index(setMinLineLastElementIndex)。同一行bottom相同会一直覆盖；
  ...
  return <>
    ...
    <section style={{ height: elementContainerHeight, overflow: 'hidden' }} ref={elementContainerRef} className={styles.mockElementContainer}>
      {mockElements.map((item, i) => <section key={item} className={`${styles.mockElement} ${styles['mockElementBg' + i]}  ${(minLineLastElementIndex === i || (!canShowAll && maxLineLastElementIndex === i)) && styles.minLineLastElement}`}>
        <>
          {item}
